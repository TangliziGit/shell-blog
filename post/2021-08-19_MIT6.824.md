...
1. 对于client的任何请求，client最少需要等待两倍RTT才能获得响应：一个RTT是client和leader间的往返，一个RTT是leader与半数follower并发AppendEntries的往返。
...
- **选主流程[发送者]**：
  ...
  3. 统计结果`VoteRequestResponse{Term, VoteGranted}`：
     - <s>`Term > currentTerm` => 成为对应节点Follower：`{Role=Follower, Term, VotedFor=nil}`</s>
     - `Votes > serverCount / 2` => 成为Leader：`{Role=Leader}`
     - 选举超时 => 重新选主
  4. （当受到同等或高等Term的AppendEntries时，转变为Follower）
- **选主流程[接收者]**：`VoteRequest{Term, CandidateID, LastLogTerm, LastLogIndex}`
  - `Term < currentTerm` => 响应投票失败
  - `Term = currentTerm` & votedFor为本人 => 检查最新Log（LogTerm&LogIndex顺序排序），投票并转变为对应Follower `{Term, Role=Follower, votedFor=CandidateID}`
  - `Term > currentTerm` => 检查最新Log（LogTerm&LogIndex顺序排序），投票并转变为对应Follower `{Term, Role=Follower, votedFor=CandidateID}`

**注意**：

- 这个RPC是幂等的。
- 当Term相同时，对votedFor的检验才有意义。因为Follower当前Term的votedFor是上一次选举时的产物，而非本次选举的。
- 可以将选主流程认为是：在`{Term, LastLogTerm, LastLogIndex, sendingTime}`上识别偏序集的过程。
- 最新的Log被定义为，有最高的Term和最高的Index。
- 会存在脑裂，但是不过半的小集群Leader不会提交Log。于是小集群Leader不会响应Client的请求，也不会执行Client的命令。
- 在网络恢复后，小集群Leader将收到大集群Leader的AppendEntries。此时更新状态，并改变角色为Follower。
...
- **触发时机**：？？？（定期AppendEntries还是Start接口）
- **复制流程[发送者]**：
  - 发送`AppendEntries{}`
  - 统计响应`AppendEntriesResponse{Term, Success}`：
...
- 为了能够更快的恢复日志，Follower需要返回足够的信息给Leader，让Leader可以以一段Log为单位来回退，而不用每次只回退一条Log条目。优化方案有很多：
  - 按Term回退：Follower返回xTerm和xIndex，xTerm是指在Follower下冲突Log的Term，xIndex是指Follower下的xTerm最远冲突点。
...
## Lecture 6 & 7 - Raft: Fault Tolerance
