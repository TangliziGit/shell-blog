...
* [计算机网络](#计算机网络)
    ...
    * [HTTP](#http)
        ...
        * [Q：如何实现跨域？](#q如何实现跨域)
        * [Q：跨域资源共享 CORS 原理](#q跨域资源共享-cors-原理)
* [操作系统](#操作系统)
    * [基础](#基础-1)
        * [Q：操作系统中进程和线程的区别？](#q操作系统中进程和线程的区别)
        * [Q：死锁的产生和避免?](#q死锁的产生和避免)
        * [Q：基本分页 & 请求分页](#q基本分页-请求分页)
...
* [语言方面](#语言方面)
    * [Rust](#rust)
        ...
        * [Q：Trait Object 对象安全](#qtrait-object-对象安全)
        * [Q：动态分发和静态分发](#q动态分发和静态分发)
        * [Q：Drop & Deref](#qdrop-deref)
        * [Q：解引用强制多态](#q解引用强制多态)
    * [Java](#java)
        * [Q：异常与错误](#q异常与错误)
        * [Q：解释一下IoC和DI](#q解释一下ioc和di)
...
3. 客户端进行证书验证，若验证失败则放弃后续访问。<br>验证成功则再次生成一个**随机字符串（premaster）**。<br>随机串经过非对称加密的公钥后，发送给服务器，**通知服务器**之后的消息用对称密钥加密。<br>客户端根据三个随机数生成对话密钥。
4. 服务器接受数据用非对称加密的密钥进行解密。<br>拿到随机串后，根据三个随机数生成对话密钥，**通知客户端**之后的消息用对称密钥加密，完成TLS1.2握手。
...
1. 自动在头信息之中，添加一个`Origin`字段。说明请求来自哪个源（协议 + 域名 + 端口）。
   ...
   - 若为简单请求，则直接请求
   - 若为非简单请求，则发送`OPTION`方法，对`Access-Control-Allow-Origin`进行预检查，而后进行正常通信。检查方法同下文。

2. 服务器返回一个正常的HTTP回应。客户端检查是否存在`Access-Control-Allow-Origin`字段。
...
- `epoll_create` 创建一个红黑树和一个链表
- `epoll_ctl`向红黑树中增加请求项，同时增加一个回调函数
- `epoll_wait`判断链表中是否存在元素，若有则返回；若无则阻塞等待
...
- 聚集索引就是以**主键**创建的索引；<br>非聚集索引就是以**非主键**创建的索引
- 聚集索引在叶子节点存储的是**数据**；<br>非聚集索引在叶子节点存储的是**主键**
- 非聚集索引会生成多个索引树
- 非聚簇索引查询时，需要扫描两次B+树：<br>第一次扫描通过非聚簇索引定位到**聚簇索引**<br>第二次扫描通过聚簇索引的值定位到**记录数据**，即回表。
...
> [Mysql RR级别依然可能丢失更新数据](https://zhuanlan.zhihu.com/p/67210493)
...
1. 读未提交：事务未提交时可触发
2. 读已提交：事务已提交时可触发
3. 可重复读：**可触发**，提交覆盖其他事务的更新，注意加锁；PostgreSQL 会回滚
4. 读未提交：不可触发
...
2. 公平锁：使用CLH方法，即虚拟队列中自旋等待前驱线程的锁释放。
